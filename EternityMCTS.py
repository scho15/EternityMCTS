...
class EternityMCTS():
    ...
    def fullSolutionCheck(cutoff,inputSolution):
        ...
        while (iteration <= cutoff and termination == False):
            ...
            if (iteration <= 16):
                if (iteration == 1):
                    ...
                    consecutivePatterns = CreateTile.findConsecutivePatternMatches(0,tileList[inputSolution[iteration-2]][1])
            ...
            if (iteration == 16):
                consecutivePatterns = CreateTile.findThreeConsecutivePatternMatches(0, 0, CreateTile.tileList[inputSolution[iteration-2]][1])
                #print(f"Matching tile list at corner is {consecutivePatterns}") # Optional Line 1
            if (iteration > 16 and iteration%16 == 1):
                consecutivePatterns = CreateTile.findConsecutivePatternMatches(CreateTile.tileList[inputSolution[iteration-17]][0],0)
            if (iteration > 16 and iteration%16 != 0 and iteration%16 != 1):
            ...
            #if (count%25000000 == 0 or (iteration <= 92 and count > 500000)):
